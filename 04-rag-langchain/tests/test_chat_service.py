...
async def test_handle_accumulates_history(llm_mock: LLMClient) -> None:
    ...
    saved = history.get_messages(USER_ID)
    assert len(saved) == 4
    assert isinstance(saved[0], HumanMessage)
    assert saved[0].content == "Первый вопрос"
    assert isinstance(saved[1], AIMessage)
    assert saved[1].content == "Ответ"
    assert isinstance(saved[2], HumanMessage)
    assert saved[2].content == "Второй вопрос"
